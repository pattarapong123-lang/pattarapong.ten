...
6. **Projects (`#projects`)** — **เปลี่ยนจาก mock data ทั้งหมดเป็นข้อมูลจริง 3 โปรเจกต์แล้ว (2026-07-26)** — ถอด mock 6 รายการเดิมออกหมด (Office-wide Hardware Rollout, Helpdesk Ticketing Optimization, Wi-Fi Infrastructure Upgrade, Active Directory Cleanup, Backup & DR Plan, Remote Support Toolkit) ทั้งจาก Supabase และ `MOCK_DATA`
   - **โปรเจกต์จริงทั้ง 3 รายการ (เรียงจากล่าสุด)**: Dohome **Buriram** → Dohome **Surat Thani** → Dohome **Amata Nakorn** — ทั้งหมดเป็น "Setup Newstore IT System" (Greenfield branch build-out) เนื้อหาเหมือนกันทุกตัวอักษร ต่างแค่ชื่อสาขา + รูปภาพ (มาจาก template เดียวกันที่ผู้ใช้ส่งมาเป็นไฟล์ .txt ภาษาไทย แปลเป็นอังกฤษ), หมวด `Infrastructure`, ติด `Featured` ทั้ง 3
...
- ตาราง (ทั้งหมดเปิด RLS พร้อม policy "public can read"):
  ...
  - `projects` — id, sort_order, title, category, featured, icon, description, tags (jsonb, ไม่ได้ใช้แสดงผลแล้วแต่ยังเก็บไว้), **`details` (jsonb — ใหม่, grouped breakdown)**, images (jsonb) — **3 แถว ข้อมูลจริงทั้งหมดแล้ว** (sort_order -2/-1/0 = Buriram/Surat Thani/Amata Nakorn, ใช้เลขติดลบเพื่อแทรกก่อนแถวเดิมโดยไม่ต้อง renumber)
