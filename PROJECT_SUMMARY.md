...
6. **Projects (`#projects`)** — **เปลี่ยนจาก mock data ทั้งหมดเป็นข้อมูลจริงแล้ว, ตอนนี้มี 7 โปรเจกต์รวม (2026-07-27)** — ถอด mock 6 รายการเดิมออกหมด (Office-wide Hardware Rollout, Helpdesk Ticketing Optimization, Wi-Fi Infrastructure Upgrade, Active Directory Cleanup, Backup & DR Plan, Remote Support Toolkit) ทั้งจาก Supabase และ `MOCK_DATA`
   ...
   - **โปรเจกต์ใหม่เพิ่ม 4 รายการ (2026-07-27) — desc สั้นอย่างเดียว ไม่มี `details` breakdown** (ผู้ใช้เลือก "short desc only" ตอนเพิ่ม) เรียงอยู่**บนสุด** (สั่ง sort_order ก่อนแล้ว แต่**ลำดับที่แน่นอนในกลุ่ม 4 นี้ยังไม่ final — ผู้ใช้บอก "add them first, sort later" จะขอจัดลำดับใหม่ทีหลัง**):
     1. **Preventive Maintenance & Hardware Upgrade — Dohome Bangna (100 PCs)** — หมวด `Maintenance`, icon 🖥️, รูปจาก `photos/Projects/pm-computer-bn/` (12 รูป)
     2. **Network Preventive Maintenance — 6 Dohome Branches** (ขอนแก่น/อุดรธานี/รังสิต/บางบัวทอง/บางนา/เพชรเกษม) — หมวด `Maintenance`, icon 🌐, รูปจาก `photos/Projects/pm-network/` (12 รูป)
     3. **IT System Restoration & Renovation — Dohome Ubon Ratchathani** — งานฟื้นฟูหลังน้ำท่วม (IT Disaster Recovery), หมวด `Infrastructure`, icon 🛠️, รูปจาก `photos/Projects/renovate-ub/` (12 รูป) — ไม่มีไฟล์ .txt ต้นทาง ผู้ใช้พิมพ์รายละเอียดในแชทแทน
     4. **IT Operations & Knowledge Transfer Support — Dohome BW Branch** — คุมสาขาชั่วคราวช่วงเปลี่ยนผ่าน มี.ค.–มิ.ย. 2565 + สอนงาน IT คนใหม่, หมวด `Support`, icon 🎓, รูปจาก `photos/Projects/tranning-bw/` (**13 รูป** — มีไฟล์พิเศษ `IMG_4814_preview.jpeg` เพิ่มจาก 12 ไฟล์เลขปกติ)
