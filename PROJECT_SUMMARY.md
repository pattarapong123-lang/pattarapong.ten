...
6. **Projects (`#projects`)** — **เปลี่ยนจาก mock data ทั้งหมดเป็นข้อมูลจริงแล้ว, ตอนนี้มี 12 โปรเจกต์รวม (2026-07-28)** — ถอด mock 6 รายการเดิมออกหมด (Office-wide Hardware Rollout, Helpdesk Ticketing Optimization, Wi-Fi Infrastructure Upgrade, Active Directory Cleanup, Backup & DR Plan, Remote Support Toolkit) ทั้งจาก Supabase และ `MOCK_DATA`
   ...
   - **โปรเจกต์ใหม่เพิ่ม 5 รายการ (2026-07-27)** เรียงอยู่**บนสุด** (สั่ง sort_order ก่อนแล้ว แต่**ลำดับที่แน่นอนในกลุ่ม 4 รายการแรกยังไม่ final — ผู้ใช้บอก "add them first, sort later" จะขอจัดลำดับใหม่ทีหลัง**):
     -4. **On-Premises Server Installation & Virtualization Infrastructure** — เพิ่มล่าสุด (2026-07-28) อยู่บนสุด, ไม่ผูกกับสาขาใดเฉพาะ, หมวด `Infrastructure`, icon 🖴, รูปจาก `photos/Projects/server-virtualization/` (**11 รูป**: โฟลเดอร์ต้นทาง `my project/server/` มีไฟล์ต้นฉบับเป็น 1-8,10,11,12 [ไม่มี 9] — เรียงเลขใหม่ให้ต่อเนื่อง 1-11 ตอนคัดลอกเข้า `photos/Projects/`) — มี `details` breakdown เต็ม (1 กลุ่ม "Server & Virtualization Setup" 4 ข้อ: Server Installation (iLO remote mgmt), Virtualization Platform Deployment (VMware/Proxmox/ZStack), Virtual Machine Management, Network Backup Integration)
     -3. **Access Point Replacement** — เพิ่มล่าสุด อยู่บนสุด, ไม่ผูกกับสาขาใดเฉพาะ (งาน Wi-Fi Infrastructure Upgrade ทั่วไป), หมวด `Infrastructure`, icon 📶, รูปจาก `photos/Projects/ap-replacement/` (12 รูป, โฟลเดอร์ต้นทาง `my project/AP/`) — มี `details` breakdown เต็ม (1 กลุ่ม "Wi-Fi Upgrade Operations" 3 ข้อ: Infrastructure Replacement (Ubiquiti UniFi → Ruijie, 50+ จุด), Dead Zone Mitigation & Site Survey, RF Optimization & Tuning)
     -2. **IT Hardware Repair & Maintenance** — เพิ่มล่าสุด อยู่บนสุด, ไม่ผูกกับสาขาใดเฉพาะ (งานซ่อมอุปกรณ์ไอทีทั่วไปในองค์กร), หมวด `Maintenance`, icon 🔧, รูปจาก `photos/Projects/it-hardware-repair/` (12 รูป, โฟลเดอร์ต้นทาง `my project/Fix/`) — มี `details` breakdown เต็ม (1 กลุ่ม "Repair & Maintenance Scope" 3 ข้อ: Printer Repair & Maintenance, Handheld Device Repair, General IT Equipment Maintenance)
     -1. **IT Asset Management** — เพิ่มล่าสุด อยู่บนสุด, ไม่ผูกกับสาขาใดเฉพาะ (ระบบ/กระบวนการบริหารทรัพย์สินไอทีของบริษัท), หมวด `Management`, icon 📋, รูปจาก `photos/Projects/it-asset-management/` (**5 รูป**: 1.png/2.png/3.png/4.jpeg/5.png — โฟลเดอร์ต้นทาง `my project/asset it/` มีไฟล์ต้นฉบับเป็น 1,2,3,4,7 [ไม่มี 5] + `6.mp4` วิดีโอที่**ไม่ได้ใช้** เพราะ modal รองรับแค่รูปภาพ ไม่รองรับวิดีโอ — เรียงเลขใหม่ให้ต่อเนื่อง 1-5 ตอนคัดลอกเข้า `photos/Projects/`) — มี `details` breakdown เต็ม (1 กลุ่ม "System & Inventory Development" 4 ข้อ: In-House Web Application Development, Inventory & Stock Management, Comprehensive IT Inventory, Visual Network & Asset Mapping)
     0. **Centralized IT Monitoring & Alerting System — Monitoring Center** — ไม่มีสาขา/ไม่ผูกกับ Dohome branch ใดเฉพาะ (เป็นระบบ Monitoring Center ส่วนกลาง), หมวด `Infrastructure`, icon 📡, รูปจาก `photos/Projects/monitoring-center/` (6 รูป, โฟลเดอร์ต้นทาง `my project/Monitor BN/`) — มี `details` breakdown เต็ม (1 กลุ่ม "Monitoring Setup" 2 ข้อ: Zabbix Infrastructure Monitoring, Uptime Kuma & Real-time Alerting)
     1. **Preventive Maintenance & Hardware Upgrade — Dohome Bangna (100 PCs)** — หมวด `Maintenance`, icon 🖥️, รูปจาก `photos/Projects/pm-computer-bn/` (12 รูป) — **อัปเดต 2026-07-27: เพิ่ม `details` breakdown เต็มแล้ว** (1 กลุ่ม "PM & Upgrade Process" 5 ข้อ: Cleaning & Maintenance, Thermal Paste Replacement, Memory Upgrade, Storage Upgrade & Data Migration, Post-PM Quality Control) แปลจากข้อความไทยที่ผู้ใช้ส่งมา — **ตอนนี้ทั้ง 8 โปรเจกต์มี `details` breakdown ครบทุกรายการแล้ว**
