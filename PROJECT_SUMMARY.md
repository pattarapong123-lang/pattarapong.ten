...
6. **Projects (`#projects`)** — **เปลี่ยนจาก mock data ทั้งหมดเป็นข้อมูลจริงแล้ว, ตอนนี้มี 8 โปรเจกต์รวม (2026-07-27)** — ถอด mock 6 รายการเดิมออกหมด (Office-wide Hardware Rollout, Helpdesk Ticketing Optimization, Wi-Fi Infrastructure Upgrade, Active Directory Cleanup, Backup & DR Plan, Remote Support Toolkit) ทั้งจาก Supabase และ `MOCK_DATA`
   ...
   - **โปรเจกต์ใหม่เพิ่ม 5 รายการ (2026-07-27)** เรียงอยู่**บนสุด** (สั่ง sort_order ก่อนแล้ว แต่**ลำดับที่แน่นอนในกลุ่ม 4 รายการแรกยังไม่ final — ผู้ใช้บอก "add them first, sort later" จะขอจัดลำดับใหม่ทีหลัง**):
     0. **Centralized IT Monitoring & Alerting System — Monitoring Center** — เพิ่มล่าสุด อยู่บนสุด, ไม่มีสาขา/ไม่ผูกกับ Dohome branch ใดเฉพาะ (เป็นระบบ Monitoring Center ส่วนกลาง), หมวด `Infrastructure`, icon 📡, รูปจาก `photos/Projects/monitoring-center/` (6 รูป, โฟลเดอร์ต้นทาง `my project/Monitor BN/`) — มี `details` breakdown เต็ม (1 กลุ่ม "Monitoring Setup" 2 ข้อ: Zabbix Infrastructure Monitoring, Uptime Kuma & Real-time Alerting)
